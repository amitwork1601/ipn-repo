...
async function loadIndex() {
  try {
    ...
    state.types = ["all", ...(data.types ?? [])];
    state.tags = ["all", ...(data.tags ?? [])];
    populateFilters();
    applyFilters();
  } catch (err) {
...
function populateFilters() {
  ...
  refs.repo.innerHTML = state.repos
    ...
    .join("");
  refs.type.innerHTML = state.types
    .map((t) => `<option value="${t}">${formatLabel(t)}</option>`)
    .join("");
  refs.tag.innerHTML = state.tags
    .map((t) => `<option value="${t}">${formatLabel(t)}</option>`)
    .join("");
}
...
function applyFilters() {
  ...
  const tag = refs.tag.value;
  const sorter = refs.sort.value;

  const filtered = state.files.filter((file) => {
    const matchesSearch =
      !searchTerm ||
      [file.title, file.summary, file.path, ...(file.tags || [])]
        .filter(Boolean)
    ...
    const matchesType =
      type === "all" || (file.type ?? "other").toLowerCase() === type.toLowerCase();

    const matchesTag =
      tag === "all" || (file.tags ?? []).includes(tag);

    return matchesSearch && matchesCategory && matchesRepo && matchesType && matchesTag;
  });
...
function renderResults() {
  ...
  refs.count.textContent = `${state.filtered.length} matching document${state.filtered.length === 1 ? "" : "s"
    }`;
  ...
  state.filtered.forEach((file) => {
    ...
    card.className = `result-card${state.activeId === file.path ? " is-active" : ""
      }`;
    card.tabIndex = 0;
    ...
    card.innerHTML = `
      ...
      <div class="result-meta">
        <span class="badge ${file.type || 'other'}">${formatLabel(file.type || 'Other')}</span>
        <span class="badge category">${formatLabel(file.category || "Other")}</span>
        ${(file.tags || []).map(tag => `<span class="badge tag">${formatLabel(tag)}</span>`).join('')}
...
refs.type.addEventListener("change", applyFilters);
refs.tag.addEventListener("change", applyFilters);
